                  ...
                  drawComponent(key, schPrevX, schPrevY)//draw a component and select into schCompModel
                    .mousedown(function (ev) {
                      if(curConnection) {
                        ...
                        [a, b] = getSchPos(ev);
                        let point = [];
                        point._x = $(this).getProps('pose').x;
                        point._y = $(this).getProps('pose').y;
              ...
              case 69://E
                ...
                if (schCompModel != undefined) {
                  if (ev.shiftKey) {
                    ...
                    for (let i = 0; i < schCompModel.length; i++) {
                      schCompModel[i].rotateBy(90);
                      scschCompModel[i].moveTo(centerX + centerY - schCompModel[i].getProps('pose').y, schCompModel[i].getProps('pose').x - centerX + centerY);
                    }
                  }
                  else {
                    for (let i = 0; i < schCompModel.length; i++) {
